import psutil, npyscreen, drawille, math
...
class CreateInterface(npyscreen.NPSApp):
    ...
    def while_waiting(self):
        ...
        sorted_processes_data = self.proc_details
        curtailed_processes_data = []
        for proc in sorted_processes_data: 
            curtailed_processes_data.append("{1: >5}  {0: <60}{2: >6.2f} % {3}{2}\
            ".format( (proc['name'][:60] + '...') if len(proc['name']) > 60 else proc['name'],
                       proc['id'],
                       proc['memory'],
                       " "*int(5*self.static_x))
            )
        self.pGrid.entry_widget.values =  curtailed_processes_data
    ...
    def main(self):
        ...
        self.cpu_chart = self.window.add(TheBox, name="CPU History", relx=90, rely=5, max_height=int(get_max_y)-10, max_width=int((get_max_x-20)*0.65)-5)
        self.pGrid = self.window.add(ButtonBox, name="Processes", relx=10, rely=5, max_height=int(get_max_y)-10, max_width=int(100*self.static_x*0.35))
    ...
    def create_chart_for_cpu_using(self, height, canvas):
        ...
        for x in range(0,self.can_width):
            for height in range(self.can_height,self.can_height-data_arr[x],-1):
                canvas.set(x,height)
        return canvas.frame(-self.can_width+10,-182,self.can_width*2,self.can_height)
